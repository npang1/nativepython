...
class LdapAuthPlugin(AuthPluginBase):
    def __init__(self, hostname, base_dn, ntlm_domain=None, authorized_groups=None):
        ...
        if authorized_groups is None:
            ...
            self._authorized_groups = TupleOf(str)(
                ['CN={group}'.format(group=group) for group in authorized_groups]
            )

    @property
    def authorized_groups(self):
        return [group.split('=')[-1] for group in self._authorized_groups]
    ...
    def authenticate(self, username, password) -> str:
        ...
        # keep the first item of a comma-separated list
        memberOfGroups = [group.split(',')[0] for group in memberOf]
